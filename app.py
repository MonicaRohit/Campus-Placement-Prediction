...
st.markdown(
    "<h5 style='text-align: center; color: white;'>Please enter student details for placement prediction</h5><br>",
...
with col2:
    ...
    hsc_s = st.selectbox("**Specialization in Higher Secondary Education(12th Grade)**",
                         ('Science', 'Commerce', 'Other'))
    if hsc_s == 'Science':
        st.write("Specialization in 12th Grade:", '**Science**')
    elif hsc_s == 'Commerce':
        st.write("Specialization in 12th Grade:", '**Commerce**')
    else:
        st.write("Specialization in 12th Grade:", '**Arts**')

    degree_p = st.number_input('**Degree Percentage**', min_value=0, max_value=100)
    st.write('Degree Percentage:', degree_p)

    degree_t = st.selectbox("**Under Graduation(Degree type)- Field of Degree Education**",
...
with col3:
    ...
    if workex == 'Yes':
        st.write("Experience:", '**Yes**')
    else:
        st.write("Experience:", '**No**')
...
df = pd.DataFrame(
    ...
    columns=['Gender', '10th%', '10th_board', '12th%', '12th_board', '12th_specialization', 'graduation%',
             'graduation_deg', 'Work_experience', 'Employee_test%', 'MBA_specialization'])
...
if st.button("Predict Placement"):
    ...
    if prediction == 0:
        st.success('The student will Not get placed')
    else:
